...
<div class="mb-6">
    <div class="flex items-center gap-2 text-slate-500 text-sm mb-1">
        ...
        <span>/</span>
        <span class="text-slate-700">Yeni Teklif</span>
    </div>
    <h1 class="text-2xl font-bold text-slate-900">Yeni Teklif</h1>
    <p class="text-slate-600 mt-1">Yeni teklif oluşturun</p>
</div>
    ...
    <form method="POST" action="{{ route('quotes.store') }}" class="space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div>
                ...
                <input type="date" name="validUntil" value="{{ old('validUntil') }}" class="form-input">
            </div>
        </div>
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium">Teklif Oluştur</button>
            <a href="{{ route('quotes.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 font-medium">İptal</a>
        </div>
    </form>
</div>
<script>
