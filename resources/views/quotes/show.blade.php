...
@include('partials.invoice-document', [
    ...
    'extraInfo' => '<p class="text-sm text-slate-600">Geçerlilik: ' . ($quote->validUntil?->format('d.m.Y') ?? '-') . '</p><p class="text-sm text-slate-600">Oluşturan: ' . e(\App\Support\QuoteCreator::displayNameForQuote($quote) ?? '-') . '</p><p class="text-sm text-slate-600">Teklifi hazırlayan: ' . e($quote->personnel?->name ?? '-') . '</p>' . ($quote->branch ? '<p class="text-sm text-slate-600">Şube: ' . e($quote->branch->name) . '</p>' : '') . '<p class="text-sm mt-2"><span class="inline-flex px-2 py-1 text-xs font-medium rounded-full ' . (($quote->status ?? '') === 'taslak' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800') . '">' . e(ucfirst($quote->status ?? '-')) . '</span></p>',
