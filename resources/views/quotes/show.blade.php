...
<div class="mb-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            @endif
            <a href="{{ route('quotes.pdf', $quote) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900 font-medium">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                PDF İndir
            </a>
            <a href="{{ route('quotes.print', $quote) }}" target="_blank" rel="noopener" class="btn-print">
...
@include('partials.invoice-document', [
    ...
    'extraInfo' => '<p class="text-sm text-slate-600">Geçerlilik: ' . ($quote->validUntil?->format('d.m.Y') ?? '-') . '</p><p class="text-sm text-slate-600">Teklifi Hazırlayan: ' . e($quote->personnel?->name ?? \App\Support\QuoteCreator::displayNameForQuote($quote) ?? '-') . '</p>' . ($quote->branch ? '<p class="text-sm text-slate-600">Şube: ' . e($quote->branch->name) . '</p>' : '') . '<p class="text-sm mt-2"><span class="inline-flex px-2 py-1 text-xs font-medium rounded-full ' . (($quote->status ?? '') === 'taslak' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800') . '">' . e(ucfirst($quote->status ?? '-')) . '</span></p>',
