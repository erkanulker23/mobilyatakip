...
function addQuoteRowWithData(item) {
    const rowEl = addQuoteRow(false);
    if (!rowEl || !item) return rowEl;
    const priceEl = rowEl.querySelector('.item-price');
    if (priceEl && item.unitPrice != null) priceEl.value = fmt(item.unitPrice);
    const qtyEl = rowEl.querySelector('.item-qty');
    ...
    if (item.productId && ts) {
        ...
        const opt = Array.from(rowEl.querySelector('.item-product').options).find(o => o.value === String(item.productId));
        if (opt?.dataset?.price) {
            priceEl.value = fmt(parseFloat(opt.dataset.price) || item.unitPrice);
        }
    }
    updateQuoteTotals();
    return rowEl;
...
function initQuoteForm() {
    ...
    if (items.length) {
        items.forEach(function(item) { addQuoteRowWithData(item); });
    } else {
        addQuoteRow();
    }
