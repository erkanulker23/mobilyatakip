...
<div class="mb-6 p-4 rounded-xl bg-emerald-50 border border-emerald-200">
    <p class="text-emerald-800 font-medium mb-2">Tedarikçilere e-posta gönderilsin mi?</p>
    <p class="text-sm text-emerald-700 mb-3">Bu satıştaki ürünlerin tedarikçilerine ({{ $suppliersWithEmail->pluck('name')->join(', ') }}) satış bildirimi e-postası gönderebilirsiniz.</p>
    <form method="POST" action="{{ route('sales.send-supplier-email', $sale) }}" class="inline">
        @csrf
        <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 font-medium">Tedarikçiye E-posta Gönder</button>
    </form>
</div>
@endif
@endif
...
@if($sale->activities && $sale->activities->isNotEmpty())
