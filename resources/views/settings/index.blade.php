...
<div x-data="{ deleteOpen: false, activeTab: 'firma', submitting: false }">
    ...
    <div x-show="deleteOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="delete-logo-title">
        ...
        <div class="relative card max-w-sm w-full p-6" @keydown.escape.window="deleteOpen = false">
            <h2 id="delete-logo-title" class="text-base font-semibold text-neutral-900">Logoyu sil</h2>
            <p class="mt-2 text-sm text-neutral-500 dark:text-slate-400">Mevcut logoyu silmek istediğinize emin misiniz?</p>
    ...
    <form method="POST" action="{{ route('settings.update') }}" enctype="multipart/form-data" @submit="submitting = true">
        ...
        <div class="flex flex-wrap gap-1 p-1 mb-6 rounded-xl bg-slate-100 dark:bg-slate-800 max-w-4xl" role="tablist">
            <button type="button" role="tab" @click="activeTab = 'firma'" :aria-selected="activeTab === 'firma'"
                :class="activeTab === 'firma' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-neutral-500 hover:bg-slate-200/60 dark:hover:bg-slate-600'"
            ...
            <button type="button" role="tab" @click="activeTab = 'seo'" :aria-selected="activeTab === 'seo'"
                :class="activeTab === 'seo' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-neutral-500 hover:bg-slate-200/60 dark:hover:bg-slate-600'"
            ...
            <button type="button" role="tab" @click="activeTab = 'sms'" :aria-selected="activeTab === 'sms'"
                :class="activeTab === 'sms' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-neutral-500 hover:bg-slate-200/60 dark:hover:bg-slate-600'"
            ...
            <button type="button" role="tab" @click="activeTab = 'paytr'" :aria-selected="activeTab === 'paytr'"
                :class="activeTab === 'paytr' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-neutral-500 hover:bg-slate-200/60 dark:hover:bg-slate-600'"
            ...
            <button type="button" role="tab" @click="activeTab = 'efatura'" :aria-selected="activeTab === 'efatura'"
                :class="activeTab === 'efatura' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-neutral-500 hover:bg-slate-200/60 dark:hover:bg-slate-600'"
            ...
            <button type="button" role="tab" @click="activeTab = 'mail'" :aria-selected="activeTab === 'mail'"
                :class="activeTab === 'mail' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-neutral-500 hover:bg-slate-200/60 dark:hover:bg-slate-600'"
            ...
            <div class="p-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        @php($addressIds = $company ? \App\Support\AddressFormat::fieldIds($company) : ['cityId' => old('cityId'), 'districtId' => old('districtId')])
                ...
                @if($company?->logoUrl)
                <div class="mt-4 pt-4 border-t border-neutral-200 dark:border-slate-600">
                    <label class="form-label">Firma Logosu</label>
                    <div class="flex flex-wrap items-center gap-4 mt-2">
                        <img src="{{ $company->logoDisplayUrl() }}" alt="Firma logosu" class="h-24 w-auto object-contain border border-neutral-200 dark:border-neutral-600 rounded-xl p-2 bg-white">
                ...
                <div class="mt-4 pt-4 border-t border-neutral-200 dark:border-slate-600">
                    ...
                    <p class="mt-1 text-xs text-neutral-500 dark:text-slate-400">PNG, JPG, max 2MB. Önerilen: 200×80px</p>
                </div>
            </div>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2 flex items-center pt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm text-neutral-700 dark:text-slate-300">Test modu</span>
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div x-show="activeTab === 'efatura'" x-cloak class="card overflow-hidden mb-6">
            ...
            <div class="p-5">
                <p class="text-sm text-neutral-500 dark:text-slate-400 mb-4">E-fatura gönderimi için GİB entegratör veya özel entegratör (örn. Fitbulut, Logo) API bilgilerini girin. UBL-TR 1.2 formatında fatura üretilir ve bu endpoint’e gönderilir.</p>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2 flex items-center pt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm text-neutral-700 dark:text-slate-300">SSL/TLS kullan</span>
                        </label>
                    </div>
                </div>
            </div>
