...
<div class="card p-6 max-w-2xl">
    ...
    <form method="POST" action="{{ route('xml-feeds.store') }}">
        ...
        <div class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">Feed Adı *</label>
                <input type="text" name="name" value="{{ old('name') }}" required
                    class="w-full rounded-lg border-slate-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">XML URL *</label>
                <input type="url" name="url" value="{{ old('url') }}" required placeholder="https://..."
                    class="w-full rounded-lg border-slate-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                @error('url')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">Tedarikçi (opsiyonel)</label>
                <p class="text-xs text-neutral-500 mb-2">İsterseniz mevcut bir tedarikçi seçin. Boş bırakırsanız ürün çekerken feed'deki tedarikçiler otomatik eşleştirilir veya yoksa oluşturulur.</p>
            ...
            <div class="flex items-start gap-3">
                ...
                <label for="create_suppliers" class="text-sm text-neutral-700">Olmayan tedarikçiler tedarikçilere kaydedilsin mi? (Ürün çekerken sistemde bulunmayan tedarikçiler otomatik eklenir)</label>
            </div>
        </div>
