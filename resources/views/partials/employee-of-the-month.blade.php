    ...
    <div class="relative px-6 py-7 sm:px-8 sm:py-8">
        <div class="flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-8">
            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-5 lg:flex-1 min-w-0">
                <div class="relative shrink-0">
                    ...
                    @if($person->photoUrl)
                        ...
                        >
                    @else
                        <div class="relative h-28 w-28 sm:h-32 sm:w-32 rounded-full eotm-photo-ring bg-gradient-to-br from-amber-200 to-amber-500 flex items-center justify-center text-3xl sm:text-4xl font-bold text-amber-950">
                            {{ $initial }}
                        </div>
                    @endif
                    <span class="absolute -bottom-1 -right-1 flex h-10 w-10 items-center justify-center rounded-full bg-amber-400 text-amber-950 shadow-lg ring-4 ring-amber-950/40" aria-hidden="true">
                ...
                <div class="text-center sm:text-left min-w-0">
                    <div class="inline-flex items-center gap-2 rounded-full bg-amber-400/15 border border-amber-300/30 px-3 py-1 mb-2">
                    ...
                    <h2 class="brand-logo text-2xl sm:text-3xl lg:text-4xl leading-tight text-white tracking-tight">
                        {{ $person->name }}
                    </h2>
                    @if($person->title)
                        <p class="mt-1.5 text-base sm:text-lg text-amber-100/85 font-medium">{{ $person->title }}</p>
                    @endif
                    <p class="mt-2 text-sm text-amber-100/70 max-w-md hidden sm:block">
            ...
            <div class="flex flex-row gap-3 lg:shrink-0 w-full sm:w-auto">
                <div class="flex-1 sm:flex-none sm:min-w-[7.5rem] rounded-2xl bg-white/10 backdrop-blur-sm border border-white/10 px-4 py-3 text-center">
                    <p class="text-[10px] font-semibold uppercase tracking-wider text-amber-200/80">Adet</p>
                    <p class="mt-0.5 text-2xl sm:text-3xl font-bold tabular-nums text-white">{{ $salesCount }}</p>
                </div>
                <div class="flex-1 sm:flex-none sm:min-w-[9rem] rounded-2xl bg-white/10 backdrop-blur-sm border border-white/10 px-4 py-3 text-center">
                    <p class="text-[10px] font-semibold uppercase tracking-wider text-amber-200/80">Ciro</p>
                    <p class="mt-0.5 text-xl sm:text-2xl font-bold tabular-nums text-white">₺{{ number_format($salesTotal, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>

        <div class="mt-5 pt-4 border-t border-white/10 flex flex-wrap items-center justify-between gap-3">
            <p class="text-xs text-amber-100/60 hidden sm:block">Performans bu ay tamamlanan satışlara göre hesaplanır.</p>
            <a
                href="{{ route('personnel.show', $person->id) }}"
                class="inline-flex items-center gap-2 rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-semibold text-amber-950 hover:bg-amber-300 transition-colors shadow-lg shadow-amber-950/20 ml-auto sm:ml-0"
            >
                Profili gör
