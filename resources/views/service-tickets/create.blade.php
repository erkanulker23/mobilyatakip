...
<div class="mb-6">
    <div class="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-sm mb-1">
        <a href="{{ route('service-tickets.index') }}" class="hover:text-slate-700 dark:hover:text-slate-300">Servis Kayıtları</a>
        <span>/</span>
        <span class="text-slate-700 dark:text-slate-300">Yeni Kayıt</span>
    </div>
    <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Yeni Servis Kaydı</h1>
    <p class="text-slate-600 dark:text-slate-400 mt-1">Servis / garanti takibi için yeni kayıt oluşturun. Satıştan veya satış olmadan oluşturabilirsiniz.</p>
</div>
    ...
    <form method="POST" action="{{ route('service-tickets.store') }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <select name="saleId" class="form-select" id="saleSelect">
                <option value="">— Satış yok, direkt müşteri seçin —</option>
                @foreach($sales as $s)
                <option value="{{ $s->id }}" data-customer="{{ $s->customerId }}">{{ $s->saleNumber }} - {{ $s->customer?->name }}</option>
                @endforeach
            </select>
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Satıştan veya satış olmadan servis kaydı oluşturabilirsiniz.</p>
        </div>
        <div>
        ...
        <div id="customerInfoCard" class="hidden rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50 p-4">
            <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Müşteri Adres Bilgileri</h3>
            <div class="space-y-1 text-sm text-slate-600 dark:text-slate-400">
                <p id="customerAddress"><span class="font-medium text-slate-500 dark:text-slate-500">Adres:</span> <span class="value">-</span></p>
                <p id="customerPhone"><span class="font-medium text-slate-500 dark:text-slate-500">Telefon:</span> <span class="value">-</span></p>
                <p id="customerEmail"><span class="font-medium text-slate-500 dark:text-slate-500">E-posta:</span> <span class="value">-</span></p>
            </div>
        </div>
        <div>
        ...
        <div>
            ...
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Sorunla ilgili fotoğraflar ekleyebilirsiniz. Birden fazla resim seçebilirsiniz.</p>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <a href="{{ route('service-tickets.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500 font-medium">İptal</a>
        </div>
    </form>
</div>
<script>
