...
<div class="mb-6">
    ...
    <h1 class="page-title">Tedarikçi Ödemesi Düzenle</h1>
    <p class="page-desc">{{ $supplierPayment->supplier?->name ?? 'Tedarikçi' }} · {{ number_format($supplierPayment->amount ?? 0, 0, ',', '.') }} ₺</p>
</div>
...
<div class="bg-white dark:bg-slate-800 p-6 max-w-2xl rounded-xl shadow-sm border border-neutral-200 dark:border-slate-700">
    <form method="POST" action="{{ route('supplier-payments.update', $supplierPayment) }}" class="space-y-5">
        ...
        <div>
            ...
            <p class="font-medium text-neutral-900 dark:text-white">{{ $supplierPayment->supplier?->name ?? '—' }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" inputmode="decimal" name="amount" required value="{{ old('amount', money($supplierPayment->amount)) }}" class="form-input money-input" placeholder="0" autocomplete="off">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <select name="paymentType" class="form-select">
                    ...
                    <option value="{{ $value }}" {{ old('paymentType', $supplierPayment->paymentType) == $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <p class="mt-1 text-sm text-neutral-500 dark:text-slate-400">Nakit, havale, kredi kartı ödemeleri kasaya çıkış olarak yansır.</p>
            </div>
        </div>
        <div>
