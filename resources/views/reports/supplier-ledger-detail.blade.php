...
<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
    <div>
        <div class="flex items-center gap-2 text-slate-500 text-sm mb-1">
            <a href="{{ route('reports.supplier-ledger') }}" class="hover:text-slate-700">Tedarikçi Cari</a>
            <span>/</span>
            <span class="text-slate-700">{{ $supplier->name }}</span>
        </div>
        <h1 class="text-2xl font-bold text-slate-900">Tedarikçi Cari Ekstre</h1>
        <p class="text-slate-600 mt-1">{{ $supplier->name }} — Hareket detayı</p>
    </div>
    <div class="flex items-center gap-3">
        <a href="{{ route('suppliers.show', $supplier) }}" class="px-4 py-2 bg-slate-200 text-slate-800 rounded-lg hover:bg-slate-300 font-medium">Tedarikçi Detay</a>
