...
@if(count($salesByRate) > 0 || count($purchasesByRate) > 0)
<div class="mt-6 bg-white rounded-xl shadow-sm border border-slate-200 p-6">
    ...
    <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div><dt class="text-sm text-slate-500">Ödenecek KDV</dt><dd class="font-bold text-slate-900">{{ number_format(collect($salesByRate)->sum('kdv') - collect($purchasesByRate)->sum('kdv'), 2, ',', '.') }} ₺</dd></div>
    </dl>
</div>
@endif
@endsection
